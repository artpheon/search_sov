...
def drop_tables():
    ...
    Base.metadata.drop_all(
        bind=e, tables=[SearchRequest.__table__, SearchResponse.__table__]
    )
...
class SearchRequest(Base):
    ...
    def __repr__(self):
        return "<SearchRequest(id='%s', Address='%s', is_searched='%s')>" % (
            self.id,
            self.Adress,
            self.is_searched,
        )
...
class SearchResponse(Base):
    ...
    def __repr__(self):
        return "<SearchResponse(self_id='%s', request_id='%s', building_type='%s')>" % (
            self.id,
            self.request_id,
            self.building_type,
        )
